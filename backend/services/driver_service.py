...
def get_driver_progress_service(driver_id):
    ...
    if goal_row.empty:
        return {"current": 0, "goal": 0, "progress_percent": 0}
    ...
    if driver_velocity.empty:
        return {"current": 0, "goal": target_earnings, "progress_percent": 0}

    current = float(driver_velocity.iloc[-1]["cumulative_earnings"])

    progress = (current / target_earnings) * 100 if target_earnings > 0 else 0
